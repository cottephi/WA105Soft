...
//
void HitRecoTask::FindHitsView( LArRawData::DataCRM* crm, int iview, std::vector<LArReco::QHit> &hits, const GeomConfig* geo_config ){
...
// find hits
//
void HitRecoTask::FindHitsCh( LArRawData::DataChan *chdata, std::vector<LArReco::QHit> &hits, float ped, float pedrms){
  // get data fragments
  size_t nfrags = 0;
...
void HitRecoTask::InitHitFinder(std::string& name)
{
  ...
  if( name.compare("AlgoMultiHit") == 0 )
  {
    ...
    fHitFinder = new AlgoMultiHit(fPreampCalib/fAdcCalib, fMultiHit_relTh1, fMultiHit_relTh2, fMultiHit_absTh1, fMultiHit_absTh2, fMultiHit_aboveT/fDt, fMultiHit_padLeft, fMultiHit_padRight);
  }
  ...
  {
    std::cerr<<"ERROR: Unknown hit algorithm "<<name<<std::endl;
    exit(-1);
  }
}
...
// assign hit truth
//
void HitRecoTask::AssignHitTruth(std::vector<LArReco::QHit> &hits0, std::vector<LArReco::QHit> &hits1){
  if(!fHasTruth) return;
