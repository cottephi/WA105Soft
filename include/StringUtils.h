...
namespace STR_UTILS
{
  ...
  // VG, comment out to remove compiler warnings
  /*
...
inline TString GetPath(TString path_full, TString* path_to, TString *name) {
 ...
 inline TString Append(std::vector<TString>& list, TString sepa) {
 ...
 inline TString Append(TString *list, int N, TString sepa) {
...
inline int Duplicates( std::vector<TString> list){
	...
	for(int i1=0; i1< (int)list.size()-1; ++i1){
	  for(int i2=i1+1; i2< (int)list.size();++i2){
