...
namespace RootUtils {
  ...
  void AddMarginBin(TH1F&,double width, TString option);
    ...
    TGraph* BuildGraphCircle(TString name, TString title, double xcenter, double ycenter,double rad, int npts=100);
    ...
    TH2Poly* CopyTH2Poly(TH2Poly*, TString name, TString title="");

	std::vector<TString> DirFileList(TString path, TString ext,TString opt);

	int FillHistoFast(TH1F &, double x, double w);
	int FillHistoFast(TH2F &, double x, double y, double w);
    int FillConstantBinWidth(TH1F &h, double x, bool FixFirstBin = false);

    
    
    
    void FillWithBinVolumeSize(TH1 &h);

    int FindBin(TAxis *,double, int=0);
	...
	double* LogSpaced(int npts, double vmin, double vmax, bool forceFirst=true);
	double* LogSpacedMulti(int npts_low, int npts_up, double vmin, double vmiddle, double vmax);	
	double* UnifSpaced(int npts, double vmin, double vmax);
	void    RebinWithBinWidthLimits(TH1F &h, double width_min=0);
