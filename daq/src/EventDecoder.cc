...
ssize_t EventDecoder::Open(std::string finname)
{
   ...
   ReadBytes( m_RunHeadBuf ); //read the m_RunHeadBuf.size=5 first entries of input file and store them in m_RunHeadBuf
   dlardaq::decode_runhead(&m_RunHeadBuf[0], m_rnh); //decode m_RunHeadBuf in m_rnh


   m_pstart = m_file.tellg(); //returns position of the cursor in input file (iostreampos object)

   // read footer
   // fast forward to the end
   m_file.seekg(-dlardaq::FileFootSz, m_file.end ); //Position the cursor at filefootsize char away from the end of the file
   m_pend = m_file.tellg(); //get the corresponding position

   ReadBytes( m_EndFootBuf ); //read the m_EndFootBuf.size=4 last entries of input file and store them in m_EndFootBuf
   dlardaq::decode_filefoot(&m_EndFootBuf[0], m_flf);//decode m_EndFootBuf in m_flf

   // rewind back to the begining after header
...
void EventDecoder::ReadEvent( std::vector<adc16_t> &adc, bool headonly )
{
   ...
   ReadBytes(m_EveDataBuf); //Load raw binary event data in m_EveDataBuf
   Decode( &m_EveDataBuf[0], m_EveDataBuf.size(), GETDCFLAG(m_evh.dq_flag), adc ); //Decode it in adc
}
...
ssize_t EventDecoder::GetEvent(size_t evnum, dlardaq::evheader_t &eh,
   ...
   //
   //
   ssize_t EventDecoder::GetEvent( dlardaq::evheader_t &eh,	std::vector<adc16_t> &adc ){
      if(m_file.is_open())
      {
         // return first event
