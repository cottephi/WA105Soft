...
void AlgoMultiHit::Algorithm()
{
  ...
  for(size_t ii=0;ii<Data.size();ii++) 
  {
	...
	//std::cout << " new hit flag "<<std::endl;
      ...
      if( ii > hit->GetTbMax() && Data[ii]<hit->GetAmplitude()-threshold2 && (minimumSample==-1 || minimum>=Data[ii])){
      ...
      hitFlag = 0;
      hit->SetTbEnd( ii-1 );	
      if ((singleHitFlag && (ii - hit->GetTbStart()) >= minWidth) || !singleHitFlag){ // valid single hit
	      Hits.push_back(hit);
      }
